...
One for distances and the other for transect names.", placement = "right", trigger = "hover"
                                     ...
                                     textInput("tlength", "Length of transects in meters (only applicable for line transects)", "100,100,100,100,100,100,100", width = "70%")%>%
                                       shinyInput_label_embed(
                                         shiny_iconlink() %>%
                                           bs_embed_popover(
                                             content = "A vector of length R containing the trasect lengths. This is ignored if using point transects.", placement = "right", trigger = "hover"
                                           )
                                       ),
...
must match number of transects. Number of columns should equal to number of covariates with one column per covariate.", placement = "right", trigger = "hover"
                                           )
                                       ),
                                     ...
                                     textInput("keyfun", "detection function", "hazard")%>%
                                       shinyInput_label_embed(
                                         shiny_iconlink() %>%
                                           bs_embed_popover(
                                             content = "One of the four detection functions: 'halfnorm', 'hazard', 'exp', or 'uniform.'. You could choose based on the table above if you ran check detection functions (normally the detection function with lowest AIC)", placement = "right", trigger = "hover"
                                           )
                                       ),
                                     ...
                                     textInput("best", "Name of the best model", "FI_FI+LU+EE+BS+OP+V", width = "70%")%>%
                                       shinyInput_label_embed(
                                         shiny_iconlink() %>%
                                           bs_embed_popover(
                                             content = "One of the model computed with covariates. You could choose based on the table above (normally the model with lowest AIC)", placement = "right", trigger = "hover"
                                           )
                                       ),
                                     ...
                                     textInput("obscolm", "Column names for observation covariates (comma delimited)", placeholder = "obsA.1,obsA.2,obsB.1,obsB.2", width = "70%")%>%
                                       ...
                                       textInput("mntype", "Survey type (removal, double or depDouble)", placeholder = "removal", width = "70%")%>%
                                         shinyInput_label_embed(
                                           shiny_iconlink() %>%
                                             bs_embed_popover(
                                               content = "Either 'removal' for removal sampling, 'double' for standard double observer sampling, or 'depDouble' for dependent double observer sampling.", placement = "right", trigger = "hover"
                                             )
                                         ),
                 ...
                 tabPanel("CEAMEC",
                          ...
                          textInput("expct", "Density must under____ per ha", "5")%>%
                            shinyInput_label_embed(
                              shiny_iconlink() %>%
                                bs_embed_popover(
                                  content = "Don't set this value lower than the background density. Background density of each cell can be seen with mouse hover over.", placement = "right", trigger = "hover"
                                )
                            ),
